...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    rotatingCube->Translate(XMVectorSet(0.0f, 40.0f, 0.0f, 1.0f));
    if (FAILED(mainScene->AddRenderable(L"RotatingCube", rotatingCube)))
    {
        return 0;
    }
    if (FAILED(mainScene->SetVertexShaderOfRenderable(L"RotatingCube", L"EnvironmentMapShader")))
    {
        return 0;
    }
    if (FAILED(mainScene->SetPixelShaderOfRenderable(L"RotatingCube", L"EnvironmentMapShader")))
    {
        return 0;
    }
